...
void init_download_class(scedownload_class* sceDownloadObj)
{
    ...
    if (res != 0xc4)
        throw formatEx<std::runtime_error>(
                "SceIpmi_4E255C31 调用失败：{:#08x}", static_cast<uint32_t>(res));
    ...
    if (res != 0)
        throw formatEx<std::runtime_error>(
                "SceDownload 初始化失败：{:#08x}", static_cast<uint32_t>(res));
}
        ...
        int type)
{
    ...
    if (res < 0)
        throw formatEx<std::runtime_error>(
                "SceDownload change_state 调用失败：{:#08x}",
                static_cast<uint32_t>(res));
    if (result < 0)
        throw formatEx<std::runtime_error>(
                "SceDownload change_state 返回错误：{:#08x}",
                static_cast<uint32_t>(result));
    if (bgdlid < 0)
        throw formatEx<std::runtime_error>(
                "SceDownload change_state 获取 bgdlid 失败：{:#08x}",
                static_cast<uint32_t>(res));
    if (reinterpret_cast<intptr_t>(params.init.addr_DC0) < 0)
        throw formatEx<std::runtime_error>(
                "SceDownload change_state DC0 失败：{:#08x}",
    ...
    if (res < 0)
        throw formatEx<std::runtime_error>(
                "SceDownload 第二次 change_state 调用失败：{:#08x}",
                static_cast<uint32_t>(res));
    if (result < 0)
        throw formatEx<std::runtime_error>(
                "SceDownload 第二次 change_state 返回错误：{:#08x}",
                static_cast<uint32_t>(result));
    buf_dc0.clear();
    buf_2e0.clear();
}
        ...
        const std::vector<uint8_t>& rif)
{
    if (pkgi_list_dir_contents("ux0:bgdl/t").size() >= 32)
        throw std::runtime_error(
                "设备上待安装的任务过多，请先在 LiveArea 通知中安装或删除"
                "部分任务，然后再继续下载。");
